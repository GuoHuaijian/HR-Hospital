...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						<div>
							<a href="<%=path %>/department/toAdd" class="btn btn-success">添加部门</a>
						</div>
						<table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            %>
								<tr class="gradeA">
									...
									<td><a href="<%=path %>/department/<%=department.getId() %>/toUpdate" class="btn btn-primary">修改</a>&nbsp;&nbsp;
										<button onclick="del(<%=department.getId() %>)" class="btn btn-danger">删除</button></td>
								</tr>
							<%
                            		}
                           		 }
						...
						<div>
							...
							<nav aria-label="Page navigation" style="margin: 0 auto; width: 240px">
								<ul class="pagination" style="margin: 0;">
									<li>
										<a href="<%=path %>/department/listPage?pageNo=<%=pe.getCurrent()-1>1?pe.getCurrent()-1:1 %>"
								...
								%>
									<li><a href="<%=path %>/department/listPage?pageNo=<%=i%>"><%=i %></a></li>
								<%
	 								 }
								%>
									<li><a href="<%=path %>/department/listPage?pageNo=<%=pe.getCurrent()+1<pe.getPages()?pe.getCurrent()+1:pe.getPages() %>"
	...
	<script>
  	function del(id){
			$.post("/department/"+id+"/delete",function (r) {
				if (r=="success"){
					parent.layer.alert('删除成功！',{icon: 1});
					window.location.href="/department/listPage?pageNo=1";
				} else {
					parent.layer.alert('删除失败！',{icon: 1});
				}
			})
	}
